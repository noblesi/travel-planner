...
<template>
  ...
  >
    ...
    <form v-else class="metadata-editor__form" @submit.prevent="submitChange">
...
<style scoped>
.metadata-editor { margin-top: 14px; }
...
.metadata-editor__open:disabled { cursor: not-allowed; opacity: 0.48; }
...
.metadata-editor__heading h3 { margin: 3px 0 0; color: #334155; font-size: 16px; }
...
.metadata-editor__fields { display: grid; gap: 10px; margin-top: 14px; }
.metadata-editor__fields label { display: grid; min-width: 0; gap: 6px; }
.metadata-editor__fields label > span { color: #64748b; font-size: 11px; font-weight: 750; }
...
.metadata-editor__fields select[aria-invalid='true'] {
  border-color: var(--color-danger); box-shadow: 0 0 0 3px var(--color-danger-soft);
}
.metadata-editor__notice,
.metadata-editor__error {
  margin: 11px 0 0; font-size: 11px; line-height: 1.55; word-break: keep-all;
}
.metadata-editor__notice { color: #64748b; }
.metadata-editor__error { color: #b91c1c; }
.metadata-editor__actions { justify-content: flex-end; margin-top: 14px; }
...
.metadata-editor__actions button:disabled { cursor: wait; opacity: 0.65; }
</style>
