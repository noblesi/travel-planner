        ...
        <div class="schedule-list">
          <div v-for="schedule in schedules" :key="schedule.order" class="schedule-item">
            ...
            <div>
              ...
              <p><span class="time-slot">{{ schedule.timeSlot }}</span></p>
            </div>
          </div>
        </div>
    ...
    <div class="bottom-grid">
      ...
      <article class="panel score-panel">
        ...
        <div v-for="score in scores" :key="score.label" class="score-row">
          ...
          <div><i :style="{ width: `${score.value}%` }" /></div>
...
.schedule-list { display: grid; gap: 10px; margin-top: 18px; }.schedule-item { display: flex; align-items: center; gap: 12px; padding: 11px 13px; border: 1px solid #afb5bd; border-radius: 5px; }.schedule-item b { display: grid; width: 25px; height: 25px; place-items: center; border-radius: 50%; background: #ff6c20; color: #fff; }.schedule-item strong { font-size: 13px; }.schedule-item p { display: flex; align-items: center; gap: 7px; margin: 5px 0 0; color: #8e949d; font-size: 11px; }.time-slot { display: inline-flex; min-height: 20px; align-items: center; padding: 0 7px; border-radius: 10px; background: var(--admin-orange-soft); color: var(--admin-orange); font-weight: 800; }.more-button { display: block; margin: 12px auto 0; padding: 7px 16px; border: 0; border-radius: 4px; background: #292d33; color: #fff; cursor: pointer; }
...
.score-panel { display: grid; align-content: start; gap: 17px; }.score-row { display: grid; grid-template-columns: 60px minmax(0, 1fr) 42px; align-items: center; gap: 10px; font-size: 12px; }.score-row div { height: 18px; overflow: hidden; border-radius: 3px; background: #f1f2f4; }.score-row i { display: block; height: 100%; background: #ed8d68; }.score-row strong { color: var(--admin-orange); font-size: 12px; text-align: right; white-space: nowrap; }
