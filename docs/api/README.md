...
- [여행 플랜 API](travel-plan-api.md): 지역 조회, 플랜 생성, 제작 페이지 조회·날짜 변경·일정 자동 저장
- [인증 API](authentication-api.md): 서버 session, CSRF, local 로그인·로그아웃 계약
- [장소 검색 API](place-search-api.md): TourAPI 키워드 검색, 장소 정규화, 오류 계약
